<table class="table table-striped table-td-valign-middle table-bordered bg-white">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <?php
        $getmulti = json_decode($data->multiple_image_json);
        foreach($getmulti as $value)
          { ?>
         <a href="<?=base_url($upload_path.$value) ?>" target="_blank">
            <img src="<?=base_url($upload_path.$value) ?>" width="75px">
         </a>
       <?php } ?>
      </td>
