                        ...
                        <div class="col-md-12">
                           <div class="row" id="add-more-sinfle-image-field">
                              ...
                              foreach($getall as $key => $value)
                                 { ?>
                                    <?=view('admin/multipleimage/add-single-image',array('value'=>$value)) ?>
                        ...
                        <div class="col-md-12">
                            <div class="row" id="add-more-multiple-image-field">
                                ...
                                if (!empty($EDITDATA->multiple_images)) {
                                    $all_multiple_images = json_decode($EDITDATA->multiple_images, true);
                                    $count = count($all_multiple_images);
                                }

                                for ($i = 0; $i < $count; $i++) {
                                    $value = $all_multiple_images[$i] ?? ['multiple_title' => '', 'multiple_sub_title' => '', 'multiple_image' => []];
                                    echo view('admin/multipleimage/add-multiple-image', ['i' => $i, 'value' => (object)$value]); 
                                }
                                ?>
                            </div>
                            <div class="col-md-12 text-right">
...
/*single image add more*/
   ...
   /*multiple*/
     $(document).ready(function() {
        var counter = $('.card-body').length; // Already existing fields count se start karein

        $('#add-more-multiple-image-btn').click(function() {
            counter++; // Har naye field ke liye unique index
            
            $.ajax({
                type: 'POST',
                url: '<?= base_url("admin/multipleimage/load_more_multiimage") ?>',
                data: {i: counter}, // Consistent counter bhejna
                success: function(data) {
                    $('#add-more-multiple-image-field').append(data.replace(/\[\?\]/g, '[' + counter + ']'));
                }
            });
        });
